...
int vol=0;  //almacena el valor del sensor de palanca 
int vol2=0; //almacena el valor del sensor de palanca para la funcion de proteccion de cambio accidental
...
//-------------****MENU****-----------
  ...
  else if( Estado == 2)
  {
    ...
    switch (contador) {
       ...
       if (tiempoActual - tiempoAnterior >= 350){
        ...
        lcd.setCursor(7, 0);lcd.print("  Vs:" );lcd.print(vol2);lcd.print("    ");
  ...
  else if( Estado == 3)
  {
    ...
    lcd.setCursor(3, 0);
      ...
      {
           if(digitalRead(botIzquierda) ==0){Sig_Estado = 4;EEPROM.update( 43,boostVel4 );}
    ...
    lcd.setCursor(3, 0);
      ...
      {
           if(digitalRead(botIzquierda) ==0){Sig_Estado = 3;EEPROM.update( 45,boostVel5 );}
...
if(digitalRead(botIzquierda) ==0){Sig_Estado = 6;EEPROM.update( 69,ajusteDel ); }
  ...
  if(Estado == 66)
  {
if(digitalRead(botIzquierda) ==0){Sig_Estado = 6;EEPROM.update( 65,ajusteTras );}
...
if(Estado == 78)
  {
 if(digitalRead(botIzquierda) ==0){Sig_Estado = 7;EEPROM.update( 101,VelInicial );}
...
if (Trastick >= Nticks201m && parcial201m == 0){
 ...
 //---- lectura sensor de palanca-------------------
   /*
   if (sensorpalanca.available()) { 
   Serial.print(sensorpalanca.read());
   }
   */
   vol = analogRead(A9)/sensibilidadPalanca - calPalanca;
   vol2 = vol;
   
 
 //---- sensa si la palanca fue soltada para rearmar el sistema
 if(vol2< rearmHI && vol2>rearmLOW ){
   estadoanteriorHI = 0;
      estadoanteriorLOW = 0;
  }
...
if (controlador == LOW){
 ...
 //-------tiempo y activacion de controlador para cambio------  
    ...
    if (vol > TriggerHi && estadoanteriorHI < rearmHI  || vol < TriggerLOW && estadoanteriorLOW > rearmLOW   ){
...
int menuANTIFALLOSLENTO(String *arrayMenu,int size)
{
 ...
 float opcion = 1;  //del 1 al 1.75 Opcion 1  //Del 2  al 2.75 opcion 2
  ...
  //Pinta los 2 primeros del menu como mucho
...
int digitalPotWrite(int value) {  //actualiza el valor en el potenciometro digital
  digitalWrite(CS, LOW);
